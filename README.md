...
|---|---:|---|
| Stockfish 18 calibration, 3,000 paired `30+0.3` games | above the tested 2450 boundary | `280-69-251` at 2450; +16.8 +/- 26.2 relative Elo and 2466.8 anchored, but the formal crossing remains open |
...
The slower confirmation campaign and its first extension have completed 3,000
games at `30+0.3`. Forklift scored above 50% at every tested rung and finished
`280-69-251` against Stockfish 2450: 52.4%, or +16.8 +/- 26.2 relative Elo. That
directly anchors the top match at 2466.8 in this pool, but its uncertainty still
includes level and it does not formally bracket the crossing. The conservative
conclusion is that Forklift is roughly in the 2450-2470 region under these exact
conditions; a 2500 rung is the next measurement.
...
crossing, the above-2450 tested boundary at the slower control, and the +303.0
+/- 37.1 relative Elo release match above.
...
The immediate sequence is: test the 2500 slow-control rung to close the
calibration bracket, confirm the retained search gains at slower time controls,
automate candidate-versus-champion testing, build a stronger NNUE v2, then
prove multi-thread strength.
